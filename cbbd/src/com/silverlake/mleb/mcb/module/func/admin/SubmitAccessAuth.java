...
@Service
public class SubmitAccessAuth extends FuncServices  
{

	private static Logger log = LogManager.getLogger(SubmitAccessAuth.class);
	...
	public MICBResponseBean process(MICBRequestBean arg0) {
		...
		try {
			...
			List<AcctAccess> subscriberAccessProfileList = checkValidAccessProfileAuthList(requestData);
			
			if(subscriberAccessProfileList == null )
			{
			...
			else
			{
					...
					vcRqData.setPending_accprofile_list(subscriberAccessProfileList);
					...
					VCResponse<AuthResponseData> vcResponseBankRef = 
							usrService.callOmniVC(
									AuthRequestData.method_auth_admin_access_maintAuth,
									vcRqData, 
									vcRpData, 
					...
					else
					{
						...
						obResponse.setSubscriber_auth_result(vcRpData.getAccprofile_result_list());
	...
	public List<AcctAccess> checkValidAccessProfileAuthList(ObProfileMaintRequest rq)
	{
		List<AcctAccess> subscriberAccessProfileList = null;
		
		
		if(rq.getAccess_list()!=null && rq.getAccess_list().size()>0 && rq.getRecordIds()!=null && rq.getRecordIds().trim().length()>0)
		{
			...
			for(String id:rcd_ids)
			{
					...
					for(AcctAccess tmp:rq.getAccess_list())
					{
						if(id.trim().equalsIgnoreCase(tmp.getPending_record_id().trim()))
						{
							if(subscriberAccessProfileList==null)
							{
								subscriberAccessProfileList = new ArrayList();
							}
							
							subscriberAccessProfileList.add(tmp);
							break;
						}
		...
		return subscriberAccessProfileList;
	}
