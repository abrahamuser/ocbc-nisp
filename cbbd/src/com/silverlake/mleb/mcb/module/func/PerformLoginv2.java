...
@Service
public class PerformLoginv2 extends FuncServices {

	private static Logger log = LogManager.getLogger(PerformLoginv2.class);
	...
	public MICBResponseBean process(MICBRequestBean arg0) {
		...
		try {
			...
			loginResponse.setPinBlockFlag(vcResponseData.getPin_block_flag());
			log.info("vc mobile : " + loginResponse.getObUser().getMobileNumber());
			
			loginResponse.setUserSecStatus(vcResponseData.getUser_sec_status());
			loginResponse.setUserSecCheck(vcResponseData.getUser_sec_check());
			
			try {
				if(vcResponseData.getRemaining_cool_off() != null && !vcResponseData.getRemaining_cool_off().isEmpty()) {
					String time = vcResponseData.getRemaining_cool_off();
					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			        sdf.setTimeZone(TimeZone.getTimeZone("UTC"));
			        Date date = sdf.parse("1970-01-01 " + time);
					log.info("remaining_cool_off : " + time);
			        log.info("in milliseconds: " + date.getTime());   
					loginResponse.setRemainingCoolOff(String.valueOf(date.getTime()));
				} else {
					loginResponse.setRemainingCoolOff(vcResponseData.getRemaining_cool_off());
				}
			} catch (Exception e) {
				e.printStackTrace();
			...
			if (checkDeviceProfileBinding) {
				...
				loginResponse.setBinded(isBinded);
				//Check Device limit if reaches max binding
				if(loginResponse.getObHeader().getStatusCode() != null &&
						(loginResponse.getObHeader().getStatusCode().equals(MiBConstant.MCB_UNTAGGED_DEVICE_AND_EXCEED_LIMIT) ||
								loginResponse.getObHeader().getStatusCode().equals(MiBConstant.MCB_UNTAGGED_DEVICE_AND_EXCEED_LIMIT_AND_NO_MOBILE))) {
					loginResponse.setLimitMaxBinding(true);
				} else {
					loginResponse.setLimitMaxBinding(false);
				}
			}

			DeviceProfile deviceProfile = deviceDao.getDevProfByDeviceIDAndCifAndStatus(deviceId, vcResponseData.getUsr_id());
			if(deviceProfile != null) {
				loginResponse.setDeviceLevel(deviceProfile.getDeviceLevel());
				loginResponse.setBindingStatus(deviceProfile.getStatus());
				
				List<ObDeviceCifBean> pendingDeviceBeanList = new ArrayList<ObDeviceCifBean>();
				if(deviceProfile.getDeviceLevel() != null && deviceProfile.getDeviceLevel().equals(MiBConstant.MIB_DEVICEPROFILE_LEVEL_PRIMARY)) {
					List<DeviceProfile> pendingBindingList = deviceDao.getPendingDevProfByCif(vcResponseData.getUsr_id());
					if(pendingBindingList != null) {
						for(DeviceProfile dp :pendingBindingList) {
							ObDeviceCifBean pendingDeviceBean = new ObDeviceCifBean();
							MapperUtil.copyFields(dp, pendingDeviceBean);
							pendingDeviceBean.setDevId(dp.getDeviceId());
							pendingDeviceBean.setDeviceStatus(dp.getStatus());
							pendingDeviceBean.setDevModel(dp.getDeviceModel());
							pendingDeviceBeanList.add(pendingDeviceBean);
						}
					}
				}
				loginResponse.setPendingDeviceList(pendingDeviceBeanList);
			}
...
//			}
			...
			if (loginResponse.getObHeader().getStatusCode() == null) {
				...
				// new binding process with approval
				// 3.9 sync device (active & pending device)
				List<DeviceProfile> deviceList = deviceDao.getDeviceByCIFAndStatus(vcResponseData.getUsr_id());
				...
				if(deviceList != null){
					...
					for(DeviceProfile syncDeviceProfile:deviceList){
				...
				syncDevice.syncDevice(appContext, requestData.getObUser().getOrgId(), requestData.getObUser().getUserId(), requestData.getUserContext().getSessionId(), arg0.getTranxID(), list_device, ipAddress);
			}
	...
	public boolean checkDeviceBindingStatus(ObAuthenticationResponse response, McbConf mibConf,
		...
		List<DeviceProfile> deviceList = deviceDao.getDeviceByCIFAndStatus(cif);
		...
		if (deviceList.size() == 0 && (mobileNo == null || mobileNo.trim().length() == 0)) {
			...
			if (!deviceBinded && deviceList.size() >= maxLimit && (mobileNo == null || mobileNo.trim().length() == 0)) {
				...
				if(deviceList != null && deviceList.size() == 1) {
					DeviceProfile leftBindedDevice = deviceList.get(0);
					// for special scenario when binded device left is pending and secondary device, user unable to proceed with binding
					// and will hit max device because the primary device has been unbind manually.
					if(leftBindedDevice.getStatus().equals(MiBConstant.MIB_DEVICEPROFILE_STATUS_PENDING)
							&& leftBindedDevice.getDeviceLevel() != null && leftBindedDevice.getDeviceLevel().equals(MiBConstant.MIB_DEVICEPROFILE_LEVEL_SECONDARY)) {
						log.info("Primary device has been unbinded left with Secondary Pending Device");
						response.getObHeader().setStatusCode(MiBConstant.MCB_UNTAGGED_DEVICE_AND_EXCEED_LIMIT);
					} else {
						response.getObHeader().setStatusCode(MiBConstant.MCB_UNTAGGED_DEVICE);
					}
