...
@Service
public class SubmitPolicyAuth extends FuncServices  
{

	private static Logger log = LogManager.getLogger(SubmitPolicyAuth.class);
	...
	public MICBResponseBean process(MICBRequestBean arg0) {
		...
		try {
			...
			List<AuthPolicy> policyList = checkValidUsrPolicyAuthList(requestData);
			...
			else
			{
					...
					vcRqData.setPending_authpol_list(policyList);
					...
					VCResponse<AuthResponseData> vcResponseBankRef = 
							usrService.callOmniVC(
									AuthRequestData.method_auth_admin_policy_maintAuth,
									vcRqData, 
									vcRpData, 
					...
					else
					{
						...
						obResponse.setSubscriber_auth_result(vcRpData.getAuthpol_result_list());
	...
	public List<AuthPolicy> checkValidUsrPolicyAuthList(ObProfileMaintRequest rq)
	{
		List<AuthPolicy> policyList = null;
		
		
		if(rq.getPolicy_list()!=null && rq.getPolicy_list().size()>0 && rq.getRecordIds()!=null && rq.getRecordIds().trim().length()>0)
		{
			
			String[] policyIds = rq.getRecordIds().trim().split(",");
			
			for(String id:policyIds)
			{
					//log.info("check id : ["+id+"]");
				
					for(AuthPolicy tmp:rq.getPolicy_list())
					{
