...
import com.silverlake.mleb.mcb.bean.ObFileBean;
 ...
 */
@Service
public class RetrieveTnCv2 extends CacheSessionFuncServices<ObTNCRequest, ObTNCResponse, ObTNCCache>{
	...
	public void processInternalWithCache(String locale, String sessionId, String trxId, ObTNCRequest requestBean, ObTNCResponse responseBean, ObTNCCache cacheBean, VCGenericService vcService) throws Exception {
		...
		TNCv2RequestData requestData = new TNCv2RequestData();
		requestData.setTnc_type(requestBean.getTncType());
		requestData.setCcy_code(requestBean.getCcyCode());
		VCResponse<TNCv2ResponseData> vcResponse = vcService.callOmniVC(
				VCMethodConstant.REST_METHODS.OTHERS_TNC_V2, 
				requestData, 
				TNCv2ResponseData.class, 
				true);
		if(processVCResponseError(vcResponse, responseBean)){
			return;
		}
		TNCv2ResponseData responseData = vcResponse.getData();
		if(responseData.getFile_list() != null){
			
			responseBean.setFileList(new ArrayList<ObFileBean>());
			
			for (File fileData : responseData.getFile_list()) {

				ObFileBean fileDta = new ObFileBean();

				fileDta.setTncContent(fileData.getData());
				fileDta.setTncType(fileData.getMime_type());
				fileDta.setTncName(fileData.getFile_name());
				fileDta.setTncFileType(fileData.getFile_type());
				fileDta.setCcyCode(fileData.getCcy_code());

				responseBean.getFileList().add(fileDta);
			...
			responseBean.setTncFileType("");
			responseBean.setCcyCode("");
		}
		
		cacheBean.setOmniResponseTnCv2(responseData);
