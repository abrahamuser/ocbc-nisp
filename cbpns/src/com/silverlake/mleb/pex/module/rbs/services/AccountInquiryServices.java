...
import com.silverlake.mleb.pex.server.socket.bean.AccountInquirySuccessResponse;
...
public class AccountInquiryServices extends DSPMsgUtil
{
	private String ip;
	private int port;
	 ...
	 */
	public AccountInquiryServices(String ip, int port, int timeoutInSecond)
	{
		this.ip = ip;
		this.port = port;
	...
	/**
	 * call when performAccountInquiry response "success"
	 * @return - EarMarkSuccessResponse - refer to RBS GOOD response mapping
	 * @throws Exception
	 */
	public AccountInquirySuccessResponse getSuccessResponse() throws Exception
	{
		DataBeanUtil dataBeanUtil = new DataBeanUtil();
		AccountInquirySuccessResponse resp = new AccountInquirySuccessResponse();
		
		if(null!=fullSuccessRespData)
		{
			resp = (AccountInquirySuccessResponse) dataBeanUtil.setFieldNamesAndByte(resp, fullSuccessRespData, "CP037");
		}
		else
		{
			resp = null;
	...
	/**
	 * call when performAccountInquiry response "error"
	...
	public String performAccountInquiry(String trxCode,String runningNumber, String accountNo,
		...
		try {
			...
			hostMsg.setField01_10_p_0(accountNo);
